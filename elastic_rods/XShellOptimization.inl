...
XShellOptimization<Object>::XShellOptimization(Object<Real> &flat, Object<Real> &deployed, const NewtonOptimizerOptions &eopts, std::unique_ptr<LOMinAngleConstraint<Object>> &&minAngleConstraint, int pinJoint, bool allowFlatActuation)
...
template<template<typename> class Object>
bool XShellOptimization<Object>::m_updateAdjointState(const Eigen::Ref<const Eigen::VectorXd> &params) {
...
Eigen::VectorXd XShellOptimization<Object>::gradp_J(const Eigen::Ref<const Eigen::VectorXd> &params, OptEnergyType opt_eType) {
    ...
    return objective.grad_p(opt_eType) + (beta / (m_l0 * m_l0)) * gradp_J_target(params);
}
...
Eigen::VectorXd XShellOptimization<Object>::pushforward(const Eigen::Ref<const Eigen::VectorXd> &params, const Eigen::Ref<const Eigen::VectorXd> &delta_p) {
...

    {
        ...
        delta_x3d = opt_3D.extractFullSolution(opt_3D.kkt_solver(opt_3D.solver, opt_3D.removeFixedEntries(m_linesearch_deployed.applyHessianPerSegmentRestlen(neg_deltap_padded, mask_dxdp).head(nd))));
...
Eigen::VectorXd XShellOptimization<Object>::apply_hess(const Eigen::Ref<const Eigen::VectorXd> &params,
    ...
    try {
        ...
        if (coeff_J != 0.0) {
            ...
            // auto b = (delta_dJ_dx3dp.head(nd) - m_d3E_w.head(nd)).eval();
            // m_delta_w_x = opt.extractFullSolution(opt.kkt_solver(opt.solver, opt.removeFixedEntries(b)));
    ...
    // Accumulate the J hessian matvec
    {
        ...
        if (coeff_J != 0.0) {
            ...
            delta_edofs_2d.tail(np) = delta_p;

            VecX_T<Real> delta_edofs_adj3d;
            delta_edofs_adj3d.setZero(nd + np);
            delta_edofs_adj3d.head(nd) = m_delta_w_x;

            VecX_T<Real> delta_edofs_3d(nd + np);
            delta_edofs_3d.head(nd) = m_delta_x3d;
            delta_edofs_3d.tail(np) = delta_p;
            ...
            for (const auto &t : objective.terms) {
                ...
                if ((opt_eType == OptEnergyType::Full) || (opt_eType == t.type)) {
                    if (t.type == OptEnergyType::ElasticBase){
                        std::cout << "Elastic base" << std::endl;
                        // VecX_T<Real> dg;
                        // dg =  t.term->delta_grad(delta_edofs_2d, m_diff_linkage_flat);
                        // std::cout << dg.size() << std::endl;
                        result +=  t.term->delta_grad(delta_edofs_2d, m_diff_linkage_flat).tail(np);
                    }
                    if (t.type == OptEnergyType::Target) {
                        std::cout << "Target" << std::endl;
                        // VecX_T<Real> dg;
                        // dg =  t.term->delta_grad(delta_edofs_adj3d, m_diff_linkage_deployed);
                        // std::cout << dg.size() << std::endl;
                        result += t.term->delta_grad(delta_edofs_adj3d, m_diff_linkage_deployed).tail(np);
                    }
                    else{
                        std::cout << "Else" << std::endl;
                        // VecX_T<Real> dg;
                        // dg = t.term->delta_grad(delta_edofs_3d, m_diff_linkage_deployed);
                        // std::cout << dg.size() << std::endl;
                        result += t.term->delta_grad(delta_edofs_3d, m_diff_linkage_deployed).tail(np);
                    }
                }
            }

            // Thanks to autodiff
            result += - (beta / (m_l0 * m_l0)) *  m_d3E_w.tail(np);
            result *= coeff_J;
        }
        // if (coeff_J != 0.0) {
