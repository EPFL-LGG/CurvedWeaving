...
void RodLinkage_T<Real_>::set(std::vector<MeshIO::IOVertex > vertices, // copy edited inside
    ...
    for (size_t i = 0; i < ne; ++i) {
        ...
        if (valence.at(e[0]) >= 4) throw std::runtime_error("The valence at "+std::to_string(e[0])+" is more than 4!");
        if (valence.at(e[1]) >= 4) throw std::runtime_error("The valence at "+std::to_string(e[1])+" is more than 4!");
...
{   
    ...
    for (size_t k = 0; k < numFullDoF; ++k) {
        ...
        if (k < r.thetaOffset()) {
            ...
            if  ((vtx <       2) && jointSensitivity[0]) {
                jointVertexSensitivity(vtx == 0,      component, 0);
            } else if ((vtx >= nv - 2) && jointSensitivity[1]) {
                jointVertexSensitivity(vtx == nv - 2, component, 1);
            } else {                                  
                dvk_dr.push_back({index_type(segmentDofOffset + k - 3 * 2 * s.hasStartJoint()), 1.0}); // Permuted Kronecker delta
            }
        }
...
VecX_T<Real_> RodLinkage_T<Real_>::gradientPerSegmentRestlen(bool updatedSource, EnergyType eType) const {
    ...
    VecX result(numExtendedDoFPSRL());
    if (m_linkage_dPC.restKappa) result.head(numDoF() + numRestKappaVars()) = gPerEdgeRestLen.head(numDoF() + numRestKappaVars());
...
template<typename Real_>
template<class F>
void RodLinkage_T<Real_>::Joint::visitInfluencedSegmentVars(const size_t joint_var_ind, F &visitor, bool restLenVar) const {
    assert(m_linkage);
