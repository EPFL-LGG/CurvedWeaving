...
void TargetSurfaceFitter::constructTargetSurface(const RodLinkage &linkage, size_t loop_subdivisions, Eigen::Vector3d scale_factors) {
    try {
        infer_target_surface(linkage, m_tgt_surf_V, m_tgt_surf_F, /* smoothing iterations */ loop_subdivisions, /* num extension layers */ 2, /* manipulate the target surface by scaling */scale_factors);
    }
