...
template<>
struct LaplacianStencil1D<RodLinkage> {
    using RodStencil = LaplacianStencil1D<ElasticRod>;
    using Vec2 = Eigen::Vector2d;
    // Call `visitor(endPtKappaVars, endPtKappas)` once for each interior
    // edge in the rod segments and once for each pair of
    // overlapping joint edges in the linkage.
    template<class F>
    static void visit(const RodLinkage &l, const F &visitor) {
...
template<>
struct LaplacianStencil1D<SurfaceAttractedLinkage> : public LaplacianStencil1D<RodLinkage> { };
