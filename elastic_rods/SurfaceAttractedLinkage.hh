...
struct SurfaceAttractedLinkage_T : public RodLinkage_T<Real_> {
    ...
    void set(const Eigen::MatrixX3d &targetV, const Eigen::MatrixX3i &targetF, const bool useCenterline, 
             const RodLinkage_T<Real2_> &linkage) {
    ...
    const Eigen::MatrixXd getTargetSurfaceVertices() const { return target_surface_fitter.getTargetSurfaceVertices(); }
    const Eigen::MatrixXi getTargetSurfaceFaces()   const { return target_surface_fitter.getTargetSurfaceFaces(); }
    const Eigen::MatrixXd getTargetSurfaceNormals() const { return target_surface_fitter.getTargetSurfaceNormals(); }
    ...
    void scaleJointWeights(Real jointPosWeight, Real featureMultiplier = 1.0, const std::vector<size_t> &additional_feature_pts = std::vector<size_t>()) {
        target_surface_fitter.scaleJointWeights(*this, jointPosWeight, featureMultiplier, additional_feature_pts);
    }
