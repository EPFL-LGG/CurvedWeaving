...
PYBIND11_MODULE(elastic_rods, m) {
    ...
    auto surface_attracted_linkage = py::class_<SurfaceAttractedLinkage, RodLinkage>(m, "SurfaceAttractedLinkage")
        ...
        .def(py::pickle([](const SurfaceAttractedLinkage &l) { 
            if (l.get_surface_path() != "")
                return py::make_tuple(l.get_surface_path(), l.get_use_centerline(), l.joints(), l.segments(), l.homogenousMaterial(), l.initialMinRestLength(), l.segmentRestLenToEdgeRestLenMapTranspose(), l.getPerSegmentRestLength(), l.getDesignParameterConfig()); 
            else 
                return py::make_tuple(l.get_surface_path(), l.get_use_centerline(), l.joints(), l.segments(), l.homogenousMaterial(), l.initialMinRestLength(), l.segmentRestLenToEdgeRestLenMapTranspose(), l.getPerSegmentRestLength(), l.getDesignParameterConfig(), l.getTargetSurfaceVertices(), l.getTargetSurfaceFaces()); 
                
                },
                        [](const py::tuple &t) {
                            if (t.size() != 9 && t.size() != 11) throw std::runtime_error("Invalid SurfaceAttractedLinkage state!");
                            if (t.size() == 9)
                                ...
                                const auto rod_linkage = RodLinkage(t[2].cast<std::vector<RodLinkage::Joint>>(),
                                                                t[3].cast<std::vector<RodLinkage::RodSegment>>(),
                                                                t[4].cast<RodMaterial>(),
                                                                t[5].cast<Real>(),
                                                                t[6].cast<SuiteSparseMatrix>(),
                                                                t[7].cast<Eigen::VectorXd>(),
                                                                t[8].cast<DesignParameterConfig>());
                                return std::make_unique<SurfaceAttractedLinkage>(t[9].cast<Eigen::MatrixX3d>(),
                                                                                 t[10].cast<Eigen::MatrixX3i>(),
                                                                                 t[1].cast<bool>(),
                                                                                 rod_linkage);
                            }
