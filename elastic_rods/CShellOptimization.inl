...
CShellOptimization<Object>::CShellOptimization(Object<Real> &flat, Object<Real> &deployed, const NewtonOptimizerOptions &eopts, std::unique_ptr<LOMinAngleConstraint<Object>> &&minAngleConstraint, int pinJoint, bool allowFlatActuation)
      ...
      m_allowFlatActuation(allowFlatActuation)
{
    ...
    objective.add("ElasticEnergyDeployed",  OET::ElasticDeployed,        std::make_shared<EEO>(m_linesearch_deployed), 1.0 / m_E0);
...
template<template<typename> class Object>
void CShellOptimization<Object>::m_forceEquilibriumUpdate() {
    ...
    try {
        ...
        if (!cr_deploy.success) throw std::runtime_error("Equilibrium solve did not converge");
    }
...
template<template<typename> class Object>
bool CShellOptimization<Object>::m_updateEquilibria(const Eigen::Ref<const Eigen::VectorXd> &newParams) {
...
// template<template<typename> class Object>
// void CShellOptimization<Object>::m_updateMinAngleConstraintActuation() {
...
template<template<typename> class Object>
bool CShellOptimization<Object>::m_updateAdjointState(const Eigen::Ref<const Eigen::VectorXd> &params) {
...
template<template<typename> class Object>
Eigen::VectorXd CShellOptimization<Object>::pushforward(const Eigen::Ref<const Eigen::VectorXd> &params, const Eigen::Ref<const Eigen::VectorXd> &delta_p) {

    m_linesearch_deployed.set(m_deployed);
    Eigen::VectorXd delta_x3d;
    ...
    m_linesearch_deployed.setDesignParameters(params);

    return delta_x3d;
}

template<template<typename> class Object>
Eigen::VectorXd CShellOptimization<Object>::gradp_J(const Eigen::Ref<const Eigen::VectorXd> &params, OptEnergyType /* opt_eType */) {
    m_updateAdjointState(params);
    Eigen::VectorXd gradient = 
        (1.0 / m_E0) * m_linesearch_deployed.grad_design_parameters(true)
...
template<template<typename> class Object>
Eigen::VectorXd CShellOptimization<Object>::gradp_J_target(const Eigen::Ref<const Eigen::VectorXd> &params) {
    m_updateAdjointState(params);
...
template<template<typename> class Object>
Eigen::VectorXd CShellOptimization<Object>::apply_hess(const Eigen::Ref<const Eigen::VectorXd> &params,
...
template<template<typename> class Object>
void CShellOptimization<Object>::dumpHessians(const std::string &hess_J_path, const std::string &hess_ac_path, Real fd_eps) {
