...
def get_all_curve_svg(file_name, top_x_list, top_y_list, bottom_x_list, bottom_y_list, joint_x_list, joint_y_list, joint_index_list_list, ribbon_index_list, joint_angle_list_list, all_joint_direction_list, upper_joint_x_list, upper_joint_y_list, lower_joint_x_list, lower_joint_y_list, thickness, use_slit, slit_orientation, default_viewBox = None, write_file = True, big_font = False, display_only = False):
...
# Parameters related to laser cutting file generation
    ...
    for list_index in range(len(top_x_list)):
        ...
        y_offset += max(max(top_y), max(bottom_y)) - min(min(top_y), min(bottom_y))
        max_x = max(max_x, max(top_x) - min(top_x))

        viewBoxWidth = max(viewBoxWidth, pm * max(max(top_x), max(bottom_x)) - min(min(top_x), min(bottom_x)))
        viewBoxHeight = max(viewBoxHeight, pm * max(max(top_y), max(bottom_y)) - min(min(top_y), min(bottom_y)))

    if not write_file:
        return [viewBoxWidth, viewBoxHeight]
    with open(file_name, "w") as f:
        if default_viewBox:
            [viewBoxWidth, viewBoxHeight] = default_viewBox


        y_offset = 0
        print(range(len(top_x_list)))
        for list_index in range(len(top_x_list)):
            ...
            viewBoxWidth = max(viewBoxWidth, pm * max(max(top_x), max(bottom_x)) - pm * min(min(top_x), min(bottom_x)))
            viewBoxHeight = max(viewBoxHeight, pm * max(max(top_y), max(bottom_y)) - pm * min(min(top_y), min(bottom_y)))

            if viewBoxWidth / viewBoxHeight < 2:
                viewBoxWidth = viewBoxHeight * 2
            else:
                viewBoxHeight = viewBoxWidth / 2

            viewBoxWidth *= 1.1
            viewBoxHeight *= 1.1
            strFile="<?xml version='1.0' encoding='UTF-8' standalone='no'?>\r\n<!DOCTYPE svg PUBLIC '-//W3C//DTD SVG 1.1//EN' 'http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd'>\r\n<svg viewBox='0 0 {} {}' overflow='visible' version='1.1' xmlns='http://www.w3.org/2000/svg'>\r\n".format(viewBoxWidth, viewBoxHeight)
            f.write(strFile)
            ...
            for index, (pCirx, pCirY, j_index, j_angle, j_direction, upper_jx, upper_jy, lower_jx, lower_jy) in enumerate(zip(joint_x, joint_y, joint_index_list, joint_angle_list, joint_direction_list, upper_joint_x, upper_joint_y, lower_joint_x, lower_joint_y)):
                ...
                f.write('<text x="{}" y="{}" text-anchor="middle" dominant-baseline="central" transform="rotate({} {} {})" fill="{}" stroke="blue" stroke-width="0.05" font-size="{}">{}</text>\r\n'.format(x, y, -j_angle / np.pi * 180, x, y, 'blue' if big_font else 'none', rHole*pm*10 if big_font else rHole*pm*3, j_index))
            ...
            f.write('<text x="{}" y="{}" text-anchor="middle" dominant-baseline="central" transform="rotate({} {} {})" fill="{}" stroke="blue" stroke-width="0.05" font-size="{}">{}</text>\r\n'.format(x, y, -joint_angle_list[0] / np.pi * 180, x, y, 'blue' if big_font else 'none', rHole*pm*10 if big_font else rHole*pm*3, ribbon_index))
            y_offset += max(max(top_y), max(bottom_y)) - min(min(top_y), min(bottom_y))
            f.write('</g>\n')
...
def get_all_curve_pattern(linkage, thickness, rod_resolution, RIBBON_NAME, image_type = 'svg', flip_angles = False, iteration_index = 0, select_ribbon_index = [], target_ribbon_width = 5, rest = True, bending_axis = 0, use_slit = False, slit_orientation = [], return_png_data = False, limits = []):
    ribbons = order_segments_by_ribbons(linkage)
    ...
    for ribbon_index in select_ribbon_index:
        ...
        if return_png_data:
            x_upper_limit = max(np.amax(top_x), np.amax(bottom_x))
            x_lower_limit = min(np.amin(top_x), np.amin(bottom_x))
            y_upper_limit = max(np.amax(top_y), np.amax(bottom_y))
            y_lower_limit = min(np.amin(top_y), np.amin(bottom_y))
            return top_x, top_y, bottom_x, bottom_y, joint_x, joint_y, [x_upper_limit, x_lower_limit, y_upper_limit, y_lower_limit]
    ...
    def set_axis_limit(high, low, axis, direction):
        span = high - low
        new_span = span / 0.9
        mid = (low + high) / 2.
        if direction == 'x':
            axis.set_xlim(mid - new_span / 2., mid + new_span / 2.)
        else:
            axis.set_ylim(mid - new_span / 2., mid + new_span / 2.)
    if image_type == 'png':
        for i, ribbon_index in enumerate(select_ribbon_index):
            top_x, top_y, bottom_x, bottom_y, joint_x, joint_y = top_x_list[i], top_y_list[i], bottom_x_list[i], bottom_y_list[i], joint_x_list[i], joint_y_list[i]
            fig, host = plt.subplots()
            host.set_aspect('equal')
            if len(limits) == 4:
                [x_upper_limit, x_lower_limit, y_upper_limit, y_lower_limit] = limits
                set_axis_limit(x_upper_limit, x_lower_limit, host, direction = 'x')
                set_axis_limit(y_upper_limit, y_lower_limit, host, direction = 'y')
            host.axis('off')
            host.plot(top_x, top_y, '-', label='shifted up', linewidth = 0.2, color = 'black')
            host.plot(bottom_x, bottom_y, '-', label='shifted down', linewidth = 0.2, color = 'black')
            host.scatter(joint_x, joint_y, s = 0.1, facecolors='black', edgecolors='black')
            fig.savefig('./{}/{}_{}.png'.format(RIBBON_NAME, ribbon_index, iteration_index), format='png', bbox_inches='tight', dpi = 400)
...
def write_per_ribbon_svg(linkage, thickness, rod_resolution, RIBBON_NAME, image_type = 'svg', flip_angles = False, iteration_index = 0, select_ribbon_index = [], target_ribbon_width = 5, rest = True, bending_axis = 0, use_slit = False, slit_orientation = []):
    ...
    viewBox = get_all_curve_svg('./{}/iter{}_ribbons{}.svg'.format(RIBBON_NAME, iteration_index, '-'.join([str(x) for x in select_ribbon_index])), top_x_list, top_y_list, bottom_x_list, bottom_y_list, joint_x_list, joint_y_list, all_joint_index, ribbon_index_list, all_joint_angle_list, all_joint_direction_list, upper_joint_x_list, upper_joint_y_list, lower_joint_x_list, lower_joint_y_list, h, use_slit, slit_orientation, write_file = False)
    print(viewBox)

    if not os.path.exists('{}/ribbons_2d'.format(RIBBON_NAME)):
            os.makedirs('{}/ribbons_2d'.format(RIBBON_NAME))

    for ribbon_index in select_ribbon_index:
        get_all_curve_svg('{}/ribbons_2d/{}.svg'.format(RIBBON_NAME, ribbon_index), [top_x_list[ribbon_index]], [top_y_list[ribbon_index]], [bottom_x_list[ribbon_index]], [bottom_y_list[ribbon_index]], [joint_x_list[ribbon_index]], [joint_y_list[ribbon_index]], [all_joint_index[ribbon_index]], [ribbon_index_list[ribbon_index]], [all_joint_angle_list[ribbon_index]], [all_joint_direction_list[ribbon_index]], [upper_joint_x_list[ribbon_index]], [upper_joint_y_list[ribbon_index]], [lower_joint_x_list[ribbon_index]], [lower_joint_y_list[ribbon_index]], h, use_slit, slit_orientation, default_viewBox = viewBox, big_font = True, display_only = True)