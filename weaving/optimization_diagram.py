...
parallelism.set_hessian_assembly_num_threads(6)
parallelism.set_gradient_assembly_num_threads(6)
...
def get_optimization_diagram_worker(model_info):
    ...
    if os.path.isfile(stage_2_data_filename) and os.path.isfile(stage_2_weight_change_filename):
        ...
        print("Stage 2 ", name)
        optimizer = initialize_stage_2_optimizer(curved_linkage, io.SURFACE_PATH, curved_save_tgt_joint_pos, None, rw, sw, fixed_boundary_joint = fixed_boundary_joints, inner_gradTol = 1e-7)
        if float(scale_joint_weight.split(', ')[0]) != -1:
